...
Astro 공개 사이트, React/Vite 기반 CMS, Rust/Axum 백엔드를 한 저장소에서 운영하는 개인 포트폴리오 웹사이트입니다. 공개 블로그·프로젝트 페이지, 관리자 대시보드, Git 기반 버전 이력, Notion 기반 CV 렌더링, PDF 생성 파이프라인을 통합해 제공합니다.
...
| 백엔드·서비스 | Rust, Axum, MongoDB, Git object DB, Notion API, Gotenberg, S3 호환 스토리지, Cloudflare Turnstile, Giscus |
| 운영 | pnpm Workspace, cargo test, Cloudflare Pages Deploy Hook, Prettier |

## 프로젝트 구조

```text
backend/
  src/              API 서버, 인증, 콘텐츠 저장, Git/Mongo 연동, 배포 로직
  api/              OpenAPI 문서
frontend/
...
```bash
pnpm dev:backend   # Rust API 서버 실행
pnpm dev:frontend  # Astro 공개 사이트 실행
pnpm dev:cms       # Vite CMS 실행
pnpm build:backend # Rust release 빌드
pnpm build:frontend
pnpm build:cms
pnpm test:backend
pnpm check:frontend
...
- `BOOTSTRAP_ADMIN_EMAIL`과 `BOOTSTRAP_ADMIN_PASSWORD`를 함께 설정한 상태로 첫 실행하면 초기 관리자 계정이 자동 생성됩니다. 초기 생성 후에는 값을 제거하는 편이 안전합니다.
