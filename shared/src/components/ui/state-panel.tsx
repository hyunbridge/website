...
export function StatePanel({
  ...
  return (
    ...
    >
      ...
      <h2 className={cn("font-semibold tracking-tight", isCompact ? "text-xl" : "text-2xl")}>
        {title}
      </h2>
      {description ? (
        <p className={cn("mt-2 text-muted-foreground", isCompact ? "text-sm" : "text-base")}>
          {description}
        </p>
      ) : null}
      {detail ? (
        <p
          className={cn("mt-4 text-sm text-muted-foreground", isCompact ? "" : "mx-auto max-w-md")}
        >
          {detail}
        </p>
      ) : null}
